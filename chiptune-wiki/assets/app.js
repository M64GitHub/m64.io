/* Chiptune Wiki viewer — navigation, search, TraxWeekly table, cheat-sheet tabs, raw-line links. No dependencies. */
(function () {
  'use strict';
  ...
  function parse(s) {
    ...
    s.trim().split(/\s+/).forEach(t => {
      if (!t) return;
      const m = t.match(/^(type|kind|platform|tag):(.+)$/i);
      if (m) f[m[1].toLowerCase()] = m[2].toLowerCase(); else terms.push(t.toLowerCase());
    });
    return { f, terms };
  }
  ...
  function resultHtml(e, terms, i) {
    const badge = '<span class="badge t-' + esc(e.type === 'tw' ? 'tw' : e.type) + '">' + esc(e.type === 'tw' ? 'TW article' : (e.kind || e.type)) + '</span>';
    let extra = '';
    if (e.type === 'tw' && e.raw) extra = ' <a class="rm" href="' + esc(ROOT + e.raw) + '" data-stop="1">read in issue ↗</a>';
    const sn = e.type === 'tw' ? esc(e.meta) : snippet(e.text, e.lc.text, terms);
    return '<a class="res' + (i === sel ? ' on' : '') + '" data-i="' + i + '" href="' + esc(ROOT + e.url) + '"><div class="rt">' + badge + ' ' + hi(e.title, terms) + (e.type !== 'tw' && e.meta ? ' <span class="rm">' + esc(e.meta) + '</span>' : '') + extra + '</div><div class="rs">' + sn + '</div></a>';
  }
  function run() {
    if (!idx) return;
    ...
    const cands = idx.filter(e => {
      if (f.type && e.type !== f.type && !(f.type === 'tw' && e.type === 'tw')) return false;
      if (f.kind && e.kind !== f.kind) return false;
      if (f.platform && !(e.platforms || []).includes(f.platform)) return false;
    ...
    if (!items.length) h = '<div class="qempty">nothing found' + (qraw && qraw.checked ? '' : ' — tick <em>raw sources</em> to search the original texts') + '</div>';
      ...
      const pages = items.filter(e => e.type !== 'tw'), tw = items.filter(e => e.type === 'tw');
      if (pages.length) h += '<div class="qgrp">pages · ' + pages.length + '</div>' + pages.map(e => resultHtml(e, terms, items.indexOf(e))).join('');
      if (tw.length) h += '<div class="qgrp">TraxWeekly articles · ' + tw.length + '</div>' + tw.map(e => resultHtml(e, terms, items.indexOf(e))).join('');
    }
    res.innerHTML = h;
    res.scrollTop = 0;
  ...
  if (tab) {
    ...
    function apply() {
      ...
      tn.textContent = n + ' / ' + rows.length + ' articles';
    }
    ...
    $$('thead th', tab).forEach(th => th.addEventListener('click', () => {
      ...
      const val = r => { const t = r.cells[k].textContent.trim(); if (k === 1) { const d = Date.parse(t); if (!isNaN(d)) return d; } const n = parseFloat(t); return (k === 3 && !isNaN(n)) ? n : t.toLowerCase(); };
